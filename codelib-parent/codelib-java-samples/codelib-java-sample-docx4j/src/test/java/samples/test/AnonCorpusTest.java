 ...
 *
 */
public class AnonCorpusTest {

	private final static String DIR_IN = System.getProperty("user.dir") + "/docs/AnonCorpus/input/";

	private final static String DIR_OUT = System.getProperty("user.dir") + "/docs/AnonCorpus/output/";

	private final static String DIR_HANDLED = System.getProperty("user.dir") + "/docs/AnonCorpus/handled/";
	...
	@Test
	public void testAnonCorpus() throws Exception {

		AnonCorpusTest corpusAnon = new AnonCorpusTest();
			...
			if (f.isDirectory()) {
				...
				if (f.getName().endsWith("docx") || f.getName().endsWith("docm")) {
					...
					try {
						...
						if (e.getMessage() != null && e.getMessage().startsWith("Ran out of patience")) {
	...
	private void handle(File fIn) throws Docx4JException {
		...
		try {
			...
			// eg dodgy docx: CustomXmlDataStoragePart cannot be cast to
